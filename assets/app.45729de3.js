...
function inputBits(api) {
  ...
  if (bits & INPUT.KICK) {
    const onTitle = api && api.game_phase && api.game_phase() === PHASE.TITLE;
    const inDemo = api && api.original_demo_flag && (api.original_demo_flag() & 0x80) !== 0;
    if (onTitle || inDemo) bits |= INPUT.START;
  }
...
function drawOriginalTitleOverlay(api) {
  const subtype = api.original_screen_subtype ? api.original_screen_subtype() : 0;
  const screenNo = api.original_screen_number ? api.original_screen_number() : 1;
  if (screenNo !== 1) return; // demo match frames render under the match view
  if (subtype <= 1) {
    drawOverlay("TECHNOS", ["原作 logo 画面（渲染接入中）", `subtype $0${subtype}`]);
  } else if (subtype <= 4 || subtype === 0x0b) {
    drawOverlay("熱血足球リーグ", [
      "PUSH START",
      "PC：Enter / J / Z＝Start",
      "手机：点「踢球」＝Start",
    ...
    drawOverlay("くにおくん", ["原作 attract 过场（渲染接入中）", "按 Start（Enter / 踢球）进入菜单"]);
  }
}
...
function render(api) {
  ...
  if (phase === PHASE.TITLE) drawOriginalTitleOverlay(api);
...
async function main() {
  ...
  function frame(now) {
    ...
    while (acc >= stepMs) { api.game_tick(inputBits(api)); acc -= stepMs; }
    render(api);
    updateSfx(api);
